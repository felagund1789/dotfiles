...
##################################################################
# Disclaimer: This script is a modified version of wifimenu, which can be found at https://github.com/podobu/wifimenu/blob/main/wifimenu. 
# The original script is licensed under GPL-3.0-or-later, and this modified version is also licensed under the same terms.
##################################################################
...
send_notification() {
	...
	notify-send --expire-time=5000 --app-name "$program_name" --category "network.$specific" --icon "$icon" --transient "$summary" "$body"
}
...
menu_main() {
	send_notification 1 1 "$tr_scanning_networks"
...
tr_main_menu_prompt='Select Wi-Fi network'
